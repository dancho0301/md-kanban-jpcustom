...
- 説明文に含まれる `http(s)` の URL はリンクとして表示され、クリックでブラウザで開きます。
- 説明文に `[設計メモ](docs/design.md)` のような Markdown リンクを書くと、クリックでそのファイルをエディタで開けます。作業の詳細を別の Markdown ファイルに書いてカードから参照する、といった使い方ができます。
  - 相対パスは**ボードファイルからの相対**で探し、見つからなければワークスペースのルートからの相対で探します。
  - `[設計メモ](docs/design.md:42)` のように末尾へ `:行番号` を付けると、その行を開きます。
  - リンク先が `http(s)` の場合はブラウザで開きます。
