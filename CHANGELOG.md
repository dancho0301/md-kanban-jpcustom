...
- カード上のサブタスク進捗をクリックするとチェックリストが開き、その場で完了を切り替えられるようにした。
- クイック追加を追加。タイトルだけ入力して `Enter` で連続追加でき、「詳細入力」で通常の入力画面に切り替えられる。
- 説明文中の `http(s)` URL をリンク化し、クリックで外部ブラウザで開くようにした(http/https のみ許可)。
- 説明文の Markdown リンク `[表示名](パス)` に対応。ローカルファイルはエディタで開き(`:行番号` 指定可)、
  相対パスはボードファイル基準 → ワークスペース基準の順に解決する。作業の詳細を別の Markdown に切り出して
  カードから参照できる。
- カード上の説明の表示/非表示をツールバーから切り替えられるようにした。表示中でも長い説明は3行で
  省略し、カードごとに「続きを表示 / 折りたたむ」で個別に開閉できる(状態は保持)。
- 開始日 `<!-- start: YYYY-MM-DD -->` を追加。新規カードには作成日が自動的に入り、編集画面で変更・
  クリアできる。カードには開始日と期限日を範囲として表示する。
- 編集画面のタグ欄に、ボードで使用中のタグを候補チップとして表示。クリックで追加/削除でき、
  入力中は入力内容で候補を絞り込む。
- 編集画面の期限日に「今日 / 明日 / 来週 / クリア」のクイック設定ボタンを追加。
- 繰り返しタスクを追加。`<!-- repeat: daily|weekly|monthly|yearly -->` を持つカードを完了扱いの列へ移動すると、
  期限日を繰り上げた次回分が元の列に自動作成される(サブタスクは未完了にリセット)。
- アーカイブしたカードを元のボード・元の列に戻す復元機能を追加(`<!-- archivedFrom: 列名 -->` を記録)。
- 元に戻す(Undo)を追加。ツールバーのボタンまたは `Ctrl+Z` で、アーカイブ・復元を含む直前の変更を取り消せる。
- 列の並べ替え(期限順 / 優先度順 / タイトル順)を列見出しの ⇅ ボタンから実行できるようにした。
- 列ごとに「常にこの順序を保つ」ルールを設定できるようにした(`<!-- sort: due -->` として列見出し直下に保存)。
  ルールのある列はカードの追加・編集・移動のたびに自動で並べ替えられ、見出しにルール名を表示する。
  ルールは ⇅ から変更・解除でき、解除時はその時点の並びを保持する。
- ボード上の右ドラッグで画面をパン(縦横スクロール)できるようにした。パン操作に右ボタンを使うため、
  ボード上では既定の右クリックメニューを抑止する(入力欄では従来どおり表示)。
- キーボード操作を拡充(矢印でフォーカス移動、`Ctrl`+矢印でカード移動、`n` でクイック追加、`Ctrl+Enter` で保存、`?` でショートカット一覧)。
